...
class Controller extends ENIP {
  ...
  async forwardOpen() {
    ...
    const data = await super.writeAndWait(
      Buffer.concat([
        MR,
        forwardOpenData,
        connectionPath
  ...
  async readWallClock() {
    ...
    const data = await this.writeAndWait(
      CIP.MessageRouter.build(GET_ATTRIBUTE_SINGLE, identityPath, []),
  ...
  async writeWallClock(date = new Date()) {
    ...
    const data = await this.writeAndWait(
      CIP.MessageRouter.build(SET_ATTRIBUTE_SINGLE, identityPath, buf),
  ...
  async _readTag(tag, size = null) {
    const data = await this.writeAndWait(
      tag.generateReadMessageRequest(size),
      'Read Tag'
    );
  ...
  async _writeTagFragmented(tag, value = null, size = 0x01) {
    value; // Do something with value here so eslint stops complaining
